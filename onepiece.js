...
function getOnePieceStat(key){
    return Number(localStorage.getItem(key)) || 0;
}

function setOnePieceStat(key, value){
    localStorage.setItem(key, value);
}

function addOnePieceGuess(){
    const total = getOnePieceStat("onepiece_total_guesses");
    setOnePieceStat("onepiece_total_guesses", total + 1);
}

function updateOnePieceBestStreak(currentStreak){
    const best = getOnePieceStat("onepiece_best_streak");

    if(currentStreak > best){
        setOnePieceStat("onepiece_best_streak", currentStreak);
    }
}
...
function showWinMessage(){
    ...
    streak++;
    addOnePieceWin();
    updateOnePieceBestStreak(streak);
    syncSupabaseWin();   // <-- paste this exact line here
 ...
 function addOnePieceWin(){
    const wins = getOnePieceStat("onepiece_total_wins");
    setOnePieceStat("onepiece_total_wins", wins + 1);
}
function giveUp(){
