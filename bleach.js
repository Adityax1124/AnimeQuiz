...
function addBleachGuess(){
    const total = getBleachStat("bleach_total_guesses");
    setBleachStat("bleach_total_guesses", total + 1);
}

function updateBleachBestStreak(currentStreak){
    const best = getBleachStat("bleach_best_streak");

    if(currentStreak > best){
        setBleachStat("bleach_best_streak", currentStreak);
    }
}
...
document.getElementById("guessCounter").innerText = `Guesses: ${guessCount}`;
    ...
    const wins = getBleachStat("bleach_total_wins");
    setBleachStat("bleach_total_wins", wins + 1);
}
