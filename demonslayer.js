...
let styleHintShown = false;

function getDemonSlayerStat(key){
    return Number(localStorage.getItem(key)) || 0;
}

function setDemonSlayerStat(key, value){
    localStorage.setItem(key, value);
}

function addDemonSlayerGuess(){
    const total = getDemonSlayerStat("demonslayer_total_guesses");
    setDemonSlayerStat("demonslayer_total_guesses", total + 1);
}

function updateDemonSlayerBestStreak(currentStreak){
    const best = getDemonSlayerStat("demonslayer_best_streak");

    if(currentStreak > best){
        setDemonSlayerStat("demonslayer_best_streak", currentStreak);
    }
}
...
function addDemonSlayerWin(){
    const wins = getDemonSlayerStat("demonslayer_total_wins");
    setDemonSlayerStat("demonslayer_total_wins", wins + 1);
}
    if(guessCount >= 6 && !arcHintShown){
        document.getElementById("arcStatus").innerText = "First Arc Hint: Available";
    }

    if(guessCount >= 9 && !styleHintShown){
        document.getElementById("styleStatus").innerText = "Breathing / BDA Hint: Available";
    }
...
function showHint(){
    ...
    if(!styleHintShown){
        ...
        if(guessCount < 9){
            alert("Breathing / Blood Demon Art hint unlocks at 9 guesses.");
            return;
        }

        document.getElementById("styleHint").innerHTML =
        `<strong>${secretCharacter.style}</strong>`;

        document.getElementById("styleStatus").innerText =
        "Breathing / BDA Hint: Revealed";
...
function showWinMessage(){
    ...
    streak++;
    addDemonSlayerWin();
    updateDemonSlayerBestStreak(streak);
    syncSupabaseWin();
...
function continueStreak(){
    ...
    document.getElementById("styleStatus").innerText = "Breathing / BDA Hint: Locked";

    document.getElementById("arcHint").innerHTML = "Unlocks at 6 guesses";
    document.getElementById("styleHint").innerHTML = "Unlocks at 9 guesses";
